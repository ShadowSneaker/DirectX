...
bool CDirectXSetup::Init(HINSTANCE HandleInstance, int CmdShow)
{
	...
	TempMesh->Location = SVector{ 0.0f, 0.0f, 0.0f };
	TempMesh->Scale = 0.1f;
	...
	Camera = new CCamera{ SVector4{0.0f, 0.0f, 4.0f, 0.0f}, 0.0f };
...
void CDirectXSetup::RenderFrame()
{
	...
	Scale.SetScale(SVector4(1.0f));
	Rotation.SetRotatePitch(TO_RADIAN(45.0f));
	Translation.SetLocation(0.0f, 0.0f, 5.0f);

	//DirectX::XMMATRIX A = DirectX::XMMatrixScaling(1.0f, 1.0f, 1.0f);
	//DirectX::XMMATRIX B = DirectX::XMMatrixRotationX(TO_RADIAN(45.0f));
	//DirectX::XMMATRIX C = DirectX::XMMatrixTranslation(0.0f, 0.0f, 5.0f);
	...
	Projection = SMatrix4::PersepctiveFovLH(TO_RADIAN(45.0f), 640.0f / 480.0f, Camera->NearClipPlane, Camera->FarClipPlane);
	CBValues.WorldProjection = World * View * Projection;

	//DirectX::XMMATRIX A = DirectX::XMMatrixPerspectiveFovLH(TO_RADIAN(45.0f), 640.0f / 480.0f, 0.001f, 100.0f);
	...
	//Camera->MoveForward(-0.00001f);
	//Camera->Rotate(0.001f);
	TempMesh->Location += SVector{ 0.0f, 0.0f, 0.000001f };
